...
//      norms_arr.pixels[i] = norms_arr.pixels[i]>0.4;
                              ...
                              matching_pyramid_t& res_maps )
{
    ...
    //mexPrintf("Deep: Done subsampling\n");
    ...
    //mexPrintf("Deep: Done pushing back\n");
    ...
    //mexPrintf("Deep: Done preparing big cells \n");
    //hash_cube(&last->grid)
    ...
    printf("time for conv atomic patches:%f\n",time_taken);
#endif
  ...
  printf("time for aggregating patches at all levels :%f\n",time_taken);
  printf("time for sparse conv :%f\n",stt/CLOCKS_PER_SEC);
  printf("time for max conv :%f\n",stm/CLOCKS_PER_SEC);
  printf("time for prepare big cells :%f\n",stb/CLOCKS_PER_SEC);
  #endif
    //mexPrintf("Deep: Done while\n");
}
...
float_image* deep_matching( image_t* img0, image_t* img1, const dm_params_t* params, full_corres_t* corres_out )
{
  ...
  //mexPrintf("Deep: Done checking params\n");
  ...
  t = clock()-t;
  double time_taken = ((double)t)/CLOCKS_PER_SEC;
  printf("time for matching pyramids:%f\n",time_taken);
  #endif
  free_layers(source);
  free_layers(target);
  ...
  t = clock()-t;
  time_taken = ((double)t)/CLOCKS_PER_SEC;
  printf("time for finding optimal matching:%f\n",time_taken);
  #endif
  ...
  t = clock()-t;
  time_taken = ((double)t)/CLOCKS_PER_SEC;
  printf("time for gathering corrs:%f\n",time_taken);
  #endif
  ...
  for(int sc0 = sr_params->min_sc0; 
    ...
    for(int sc1 = sr_params->min_sc1; 
      ...
      for(int rotation = sr_params->min_rot;  
        ...
        if( verbose ) 
          fprintf(stderr, "processing scale = (x%g, x%g) + rotation = %d deg (code %d)...\n",
